...
Fran is a collection of types and functions for composing richly interactive animations. All of these types and functions come to gether based on some shared assumptions which allow for the ability to "model" an animation. We are able to do so by imagining all variables as a function over time.

For example we can model the path a bone tavels in an animation as a function: F(t) -> Vector3, where t is a valid number between the range of 0 and the length of the animation. How t = Vector3 isn't relevant, however the ability to model that transformation as a function over time is extremely powerful.
